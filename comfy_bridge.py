...
Simple img2img workflow — no custom nodes required.
"""
...
def _build_workflow(char_key, image_name):
    """Simple img2img — no custom nodes needed."""
    ...
    return {
        ...
        "8": {
            ...
            "inputs": {"images": ["7", 0], "filename_prefix": f"kiosk_{char_key}"},
        },
    }

def generate_character(frame, char_key, timeout=60):
    if not is_comfy_running():
        return None, "ComfyUI not running"
    try:
        img_name = _upload_image(frame)
        workflow = _build_workflow(char_key, img_name)
...
class ComfyBridge:
    def __init__(self):
        ...
        print(f"[{'OK' if self.available else 'WARN'}] ComfyUI {'connected' if self.available else 'not running — start ~/start_comfyui.sh'}")
